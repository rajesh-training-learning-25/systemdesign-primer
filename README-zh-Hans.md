...
可用性通常用正常运行时间（或停机时间）来量化，即服务可用时间的百分比。 
可用性通常通过 9 的数量来衡量 -- 比如一个 99.99% 可用性的服务，可被称为四个9。

#### 99.9% 可用性 - 三个9

|时长|可接受的停机时间|
|---|---|
|每年停机时间|8 小时 45 分种 57 秒|
|每月停机时间|43 分种 49.7 秒|
|每周停机时间|10 分钟 4.8 秒|
|每天停机时间|1 分钟 26.4 秒|
...
如果一个服务由多个容易发生故障的组件组成，那么服务的整体可用性取决于组件是串行还是并行。

##### 串行

当可用性不足 100% 的两个组件串行排列时，整体可用性会降低：

```
Availability (Total) = Availability (Foo) * Availability (Bar)
```
如果 Foo 和 Bar 两个组件的可用性均为 99.9% ，则串行排列时，整体可用性为 99.8% 。

##### 并行

当可用性不足 100% 的两个组件并行排列时，整体可用性会得到提升：

```
Availability (Total) = 1 - (1 - Availability (Foo)) * (1 - Availability (Bar))
```
如果 Foo 和 Bar 两个组件的可用性均为 99.9% ，则并行排列时，整体可用性为 99.9999% 。
